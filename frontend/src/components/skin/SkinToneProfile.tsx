    ...
    <section className="rounded-[1.25rem] border border-black/8 bg-white/78 p-5 shadow-[0_14px_34px_rgba(17,24,39,0.07)] backdrop-blur">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold tracking-tight sm:text-2xl">Color signal</h2>
        </div>
      ...
      {swatches.length || undertone ? (
        <>
          {undertone && (
            <div className="mt-5 inline-flex rounded-full border border-black/8 bg-[#f8fafc] px-3 py-1.5 text-sm font-medium capitalize">
              {undertone} undertone
            </div>
          )}
          <div className="mt-5 grid grid-cols-3 gap-3 sm:grid-cols-5">
            {swatches.map((item) => {
              ...
              return (
                <div key={item.key} className="rounded-2xl border border-black/6 bg-[#f8fafc] p-3 text-center">
                  <span
